...
def expose(name_or_function=None):
    # Deal with '@eel_flask.expose()' - treat as '@eel_flask.expose'
    if name_or_function is None:
        return expose

    if type(name_or_function) == str:   # Called as '@eel_flask.expose("my_name")'
        ...
        for name in files:
            ...
            try:
                with open(os.path.join(root, name), encoding='utf-8') as file:
                    contents = file.read()
                    expose_calls = set()
                    finder = rgx.findall(r'eel_flask\.expose\(([^\)]+)\)', contents)
                    for expose_call in finder:
                        ...
                        msg = "eel_flask.expose() call contains '(' or '='"
...
def start(*start_urls, **kwargs):
    ...
    def run_lambda():
        ...
        for route_path, route_params in BOTTLE_ROUTES.items():
            ...
            flask_app.add_url_rule(route_path, view_func=route_func, **route_kwargs)

        websocket.add_url_rule('/eel_flask', _websocket, f=_websocket)

        from gevent import pywsgi
        from geventwebsocket.handler import WebSocketHandler
        server = pywsgi.WSGIServer((HOST, _start_args['port']), flask_app, handler_class=WebSocketHandler)
...
BOTTLE_ROUTES = {
    "/eel_flask.js": (_eel, dict()),
    **urls,
    "/<path:path>": (_static, dict()),
    # "/eel_flask": (_websocket, dict(apply=[wbs.websocket])),
}
