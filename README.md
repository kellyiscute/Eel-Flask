...
<html>
  <head>
    ...
    <!-- Include eel_flask.js - note this file doesn't exist in the 'web' directory -->
    <script type="text/javascript" src="/eel.js"></script>
...
# Set web files folder and optionally specify which file types to check for eel_flask.expose()
#   *Default allowed_extensions are: ['.js', '.html', '.txt', '.htm', '.xhtml']
eel_flask.init('web', allowed_extensions=['.js', '.html'])

@eel_flask.expose                         # Expose this function to Javascript
def say_hello_py(x):
    print('Hello from %s' % x)

say_hello_py('Python World!')
eel_flask.say_hello_js('Python World!')   # Call a Javascript function

eel_flask.start('hello.html')             # Start (this blocks and enters loop)
```
...
def my_other_thread():
    while True:
        print("I'm a thread")
        eel_flask.sleep(1.0)                  # Use eel_flask.sleep(), not time.sleep()

eel_flask.spawn(my_other_thread)

eel_flask.start('main.html', block=False)     # Don't block on this call
